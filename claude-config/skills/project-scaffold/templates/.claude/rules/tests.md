...
- E2E tests use the `playwright-cli` skill (Bash-invoked `playwright-cli` commands, not the Playwright MCP). Snapshots land on disk under `.playwright-cli/` and are read on demand instead of being pushed into context — far fewer tokens per session than the MCP server.
